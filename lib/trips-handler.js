import { assertNoNewDuplicatePlaces } from '../public/js/place-identity.js';
...
export function createTripsHandler(store) {
  return async function handler(req, res) {
    ...
    try {
      ...
      if (req.headers['x-trip-edit'] !== '1') {
        return res.status(403).json({ message: '자물쇠를 눌러 편집 모드를 켜주세요.' });
      }
      ...
      // Old clients do not understand routes. Refuse destructive legacy writes;
      // the revision check still protects against routes created after this read.
      const current = await store.read();
      if (body.revision !== current.revision) throw new BlobPreconditionFailedError();
      try { assertNoNewDuplicatePlaces(current.places, data.places); }
      catch (error) { return res.status(400).json({ message: error.message }); }
      if (body.routes === undefined && current.routes?.length) {
        return res.status(409).json({ message: 'Route 기능이 업데이트되었습니다. 페이지를 새로고침한 후 다시 저장해주세요.' });
      }
